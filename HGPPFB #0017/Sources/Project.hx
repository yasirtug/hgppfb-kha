...
class Project{
	...
	public function new() {
		...
		trace("total velocity:" + Vec2.div(totalMomentum, totalMass));
	}
	...
	function update(): Void
	{
		...
		while(--i >= 0){
			var o = bodies[i];
			var dp = Vec2.sub(o.center, bigBody.center);
			var dr = o.r - outR;
			var overlap = dp.len() + dr;
			if(overlap > 0.0){
				var normal = Vec2.div(dp, dp.len());
				var totalInvertMass = o.invertMass + bigBody.invertMass;
				var invMassRatioO = o.invertMass / totalInvertMass;
				var invMassRatioBig = bigBody.invertMass / totalInvertMass;
				
				//eliminate overlapping
				o.center = Vec2.sum(o.center, Vec2.pro(normal, -invMassRatioO * overlap));
				bigBody.center = Vec2.sum(bigBody.center, Vec2.pro(normal, invMassRatioBig * overlap));
